...
rec=cur1.fetchall()
###
Q="""select * from invoice where Payment_Satus='Notpaid'"""
cur1.execute(Q)
rec2=cur1.fetchall()
print("""<!DOCTYPE html>
<html lang="en">
...
.payment-heading {
  ...
  margin-top: -18px; /* Adjusting top margin */
}
...
print("""
        ...
        <div id="Sidebar" class="d-flex flex-column  offcanvas-md offcanvas-start " >
           <div>
            ...
            <li><div class="dropdown">
                <a data-toggle="dropdown"id="op" style="background-color: #23395f; color: white; margin-left: -10px; margin-right: -8px; border-left:3px solid orange;"><i class="fa fa-users"  style="color: #CFEEFF;"></i> Client<span class="caret" style="margin-top: 10px;"></span></a>
                <ul class="dropdown-menu">
...
print("""
    ...
    <div class="centered-form-container" id="paymentForm" style="background-color: #fff; border-radius:10px;box-shadow: rgba(67, 71, 85, 0.27) 0px 0px 0.25em, rgba(90, 125, 188, 0.05) 0px 0.25em 1em; padding:30px">
        ...
        <h3 class="payment-heading">Payment Form</h3>
        <!-- Your payment form goes here -->
        <form method="post" enctype="multipart/form-data">
    <div class="form-group">
        <label for="payment_id">Client ID</label>
        <input type="number" name="payment_CID" id="payment_CID" class="form-control" readonly>
    </div>
    
        
        <input type="hidden" name="payment_invoice" id="payment_invoice" class="form-control" readonly>
    ...
    <div class="form-group">
        <label for="payment_id">Client ID</label>
        <input type="text" name="payment_organization" id="payment_organization" class="form-control" readonly>
    </div>
    <div class="form-group">
        <label for="payment_Amount">Amount:</label>
        <input type="number" name="payment_Amount" id="payment_Amount" class="form-control" readonly>
    </div>
    <div class="form-group" id="method">
        <label for="payment_Method">Payment Method:</label>
        <select name="payment_Method" id="payment_Method" class="form-control" onchange="toggleReferenceNumber()" required>
            <option selected diable></option>
    ...
    <div class="form-group" id="reference_number_div" style="display: none;">
        <label for="reference_number">Transaction ID</label>
        <input type="text" name="reference_number" id="reference_number" class="form-control">
    </div>
    <input type="submit" name="confirmPayment" value="Confirm Payment" class="btn btn-success form-control">
</form>
    </div>
                """)
html_rows = ""
for row in rec2:
    html_rows += f"""
                            ...
                            <tr>
                                ...
                                <td>{row[4]}</td>
                                <td>{row[9]}</td>
                                <td><button class="btn btn-info" onclick="openPaymentForm('{row[0]}', '{row[9]}','{row[4]}','{row[1]}','{row[12]}')">Approve</button></td>
                            </tr>
"""
print(f"""
               <div class="row">
...
print("""
    ...
    //------------------------------------------------Sayary------------------------------------//
        ...
        if (paymentMethod === "Net_Banking") {
          ...
          function openPaymentForm(CID, Amount,name,organization,invoice_no) {
            // Populate payment form fields
            document.getElementById('payment_CID').value = CID;
            document.getElementById('payment_Amount').value = Amount;
            document.getElementById('payment_Name').value = name;
            document.getElementById('payment_organization').value = organization;
            document.getElementById('payment_invoice').value =invoice_no ;
...
Approve=form.getvalue('confirmPayment')
if Approve != None:
    CID=form.getvalue('payment_CID')
    payment_name=form.getvalue('payment_name')
    payment_organization=form.getvalue('payment_organization')
    payment_Amount=form.getvalue('payment_Amount')
    payment_Method=form.getvalue('payment_Method')
    reference_number=form.getvalue('reference_number')
    payment_invoice=form.getvalue('payment_invoice')
    ...
    cur = con.cursor()
    IN="""INSERT INTO `income`(`year`, `Month`, `payment_CID`, `payment_name`, `payment_organization`, `payment_Amount`, `payment_Method`, `Transaction_ID`,invoice_no,Date ) VALUES ('%s','%s','%s','%s','%s','%s','%s','%s','%s','%s')"""%(year,Month_name,CID,payment_name,payment_organization,payment_Amount,payment_Method,reference_number,payment_invoice,todays_date)
    cur.execute(IN)
    con.commit()
    UP="update invoice set Payment_Satus='Paid' where CID='%s'"%(CID)
    cur.execute(UP)
    con.commit()
    print("""<script>
