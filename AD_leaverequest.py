...
cur2=con2.cursor()
q2="""select * from leave_table where Status='new' and Department ='HR'"""
cur2.execute(q2)
rec1=cur2.fetchall()
print("""<!DOCTYPE html>
<html lang="en">
    ...
    <div id="Sidebar_cover2">
        ...
        <div id="Sidebar" class="d-flex flex-column  offcanvas-md offcanvas-start " >
           <div>
            ...
            <li><div class="dropdown">
                <a data-toggle="dropdown"  id="op" style="background-color: #23395f; color: white; margin-left: -10px; margin-right: -8px; border-left:3px solid orange;" ><i class="fa fa-person-walking-arrow-right" style="color: #CFEEFF;"></i></span>Leave<span class="caret" style="margin-top: 10px;"></span></a>
                <ul class="dropdown-menu">
    ...
    <li><a href="index.py" id="LOGOUT_SM" class="d-md-none"> <i class="glyphicon glyphicon-log-out log_icons "></i class="glyphicon glyphicon logout"> Logout</a></li>
        ...
        <div class="flex-fill" id="first_content_box">
            ...
            <div class="container-fluid d-none d-lg-block d-xl-block " id="Dashbord_Head" ><!--Time-->
                ...
                <div class="container-fluid" id="Leaves_requset"><!--Leave_requset table-->
                     <div class="row">
                        <div class="col-md-2"></div>
                        
                        <div class="col-md-10">
                          <h3 style="font-family: verdana;font-weight: bold;font-size: 20px;color: rgb(0, 208, 255);">Leave Request Details</h3>
                    <table class="table">
                        <thead>
                            <tr>
                                <th>EmployeeId</th>
...
for j in rec1:
    print("""
                            ...
                            <form method="post" enctype="multipart/form-data">
                                <td style="display:none"><input type="number" value="%s" style="border:none" name="LID" readonly></td>
                                <td>%s</td>
                                <td>%s</td>
                                <td>%s</td>
                                <td>%s</td>
                                ...
                                <td>%s</td>
                                <td>%s</td>
                                <td style="display:flex"><input type="submit" class="btn btn-success"  name="approve" value="approve">
                                <input type="submit" class="btn btn-danger" name="reject" value="reject"></td>
                                </form>
                            </tr>
                            
                        """%(j[0],j[1],j[2],j[3],j[4],j[5],j[6],j[7],j[8],j[12],j[9]))
print("""
                        </tbody>
                    </table>
                        </div>
...
if approve !=None:
    ...
    cur2 = con2.cursor()
    #######
    q2 = """update leave_table set Status='%s' where LID='%s'""" %('Approved',LID)
    cur2.execute(q2)
    con2.commit()
if reject !=None:
    LID = form.getvalue("LID")
    ...
    cur2 = con2.cursor()
    #######
    q2 = """update leave_table set Status='%s' where LID='%s'""" % ('reject', LID)
    cur2.execute(q2)
    con2.commit()
