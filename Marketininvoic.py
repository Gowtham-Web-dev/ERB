...
####invoice tabel
    ...
    cur=con.cursor()
    q="""select * from invoice where Status='Invoiced' And EID='%s'"""%ID
    cur.execute(q)
...
if role=='Manager':
    con = pymysql.connect(host="localhost", user="root", password="", database="erp")
    cur = con.cursor()
    q = """select * from invoice where Status='Invoiced'"""
    cur.execute(q)
    rec1 = cur.fetchall()
print("""<!DOCTYPE html>
    <html lang="en">
                ...
                <div id="Sidebar" class="d-flex flex-column  offcanvas-md offcanvas-start " >
                   <div>
                    ...
                    <h3 class="Role">Role</h3>
                        ...
                        <li><div class="dropdown">
                            <a data-toggle="dropdown" id="op" style="background-color: #23395f; color: white; margin-left: -10px; margin-right: -8px; border-left:3px solid orange;"><i class="fa fa-file-invoice" style="color: #CFEEFF;" ></i>Invoice<span class="caret" style="margin-top: 10px;"></span></a>
                            <ul class="dropdown-menu">
            ...
            <div id="Secount_continer_box">
                      <div class="row">
                        <div class="col-md-2"></div>
                            <div class="col-md-10">
                             <h3 style="font-family: verdana;font-weight: bold;font-size: 20px;color: rgb(0, 208, 255);">New Invoice</h3>
                                <table class="table">
                                    <thead>
                                    <th>CID</th>
                                    <th>Invoice No</th>
    ...
    print("""
                                        <tr>
                                            <form method="post" enctype="">
                                                <td><input type="number" name="cid" value="%s" style="border: none;width:40px"readonly></td>
                                                <td>%s</td>
                                                <td>%s</td>
                                                <td>%s</td>
                                                <td>%s</td>
                                                <td>%s</td>
                                                <td>%s</td>
                                                <td><a href="%s" download>Invoice</td>
                                                <td id='a1'>%s</td>
                                                <td><input type="submit" name="approved" value="Approve" class="btn btn-success"></td>
                                            </form>
                                        </tr>
                                        """%(J[0],J[12],J[1],J[4],J[6],J[7],J[8],f,J[18]))
print("""
                                    </tbody>
                                </table>
...
if approved !=None:
    ...
    cur2 = con2.cursor()
    UP= """update invoice set Status='approved',Approved_date='%s' where CID='%s'"""%(Approved_date,cid)
    cur2.execute(UP)
    con2.commit()
    print("""<script>alert("Status Updated")</script>""")
